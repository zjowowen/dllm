#!/usr/bin/env python3
"""
Stable checkpoint sweep for OneFlow text-only PT/SFT checkpoints.

For each checkpoint under --ckpt_root:
1) Runs multi-seed Eq(7) loss evaluation.
...
LOSS_EVAL_SCRIPT = os.path.join(REPO_ROOT, "scripts", "oneflow", "eval_text_only_loss.py")
PROMPT_EVAL_SCRIPT = os.path.join(REPO_ROOT, "scripts", "oneflow", "eval_text_only_prompts.py")
...
def _build_markdown(summary: dict) -> str:
    args = summary["args"]
    lines: list[str] = []
    lines.append("# OneFlow Checkpoint Sweep Summary")
    lines.append("")
