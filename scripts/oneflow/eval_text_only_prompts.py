#!/usr/bin/env python3
"""
Batch prompt evaluation for OneFlow text generation.
...
class ScriptArguments:
    ...
    output_dir: str = "outputs/oneflow_text_prompt_eval"
    seed: int = 42
...
def _build_markdown(records: list[dict], model_dir: str, prompts_file: str) -> str:
    ...
    lines = []
    lines.append("# OneFlow Batch Prompt Eval Report")
    lines.append("")
    ...
    lines.append("")
    lines.append("| id | category | expected_pass | output_preview |")
    lines.append("|---|---|---:|---|")
    for r in records:
        ...
        if len(preview) > 100:
            preview = preview[:97] + "..."
        lines.append(f"| {r.get('id')} | {r.get('category')} | {ps} | {preview} |")
    lines.append("")
    return "\n".join(lines)
