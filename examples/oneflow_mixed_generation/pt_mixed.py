"""
OneFlow mixed-generation pretraining from offline WebDataset latents shards.

Jointly trains text insertion loss and image flow matching loss with
`mixed_generation_prob` controlling the image-bearing sample ratio.

Corresponds to Phase 2a in doc/oneflow/text_image_interleaved_review_zh.md.

Example:
  accelerate launch --config_file scripts/accelerate_configs/npu_ddp.yaml \
    examples/oneflow_mixed_generation/pt_mixed.py \
    --output_dir /tmp/oneflow_mixed_gen \
...
import dllm
from dllm.pipelines.oneflow_mixed_generation.trainer import OneFlowMixedGenerationTrainer
...
@dataclass
class TrainingArguments(OneFlowMixedGenerationTrainer.OneFlowMixedGenerationConfig):
    output_dir: str = None
    ...
    # Core mixed-generation parameter
    mixed_generation_prob: float = 0.5
...
def expand_shards(spec: str) -> list[str]:
    ...
    if os.path.isdir(spec):
        out = sorted(
            os.path.join(spec, n) for n in os.listdir(spec) if n.endswith(".tar")
        )
        if not out:
            raise FileNotFoundError(f"No .tar shards found in dir: {spec}")
        return out
    if os.path.isfile(spec) and spec.endswith(".txt"):
        out = [l.strip() for l in open(spec) if l.strip()]
        if not out:
...
class WDSLatentsIterableDataset(torch.utils.data.IterableDataset):
    def __init__(self, *, shards, tokenizer, latent_key, caption_key, meta_key,
                 meta_input_ids_field, use_precomputed_ids, max_caption_tokens,
                 shardshuffle, shuffle_buffer, max_samples, seed):
        super().__init__()
        ...
        if not self._my_shards:
            raise ValueError(f"Shard split empty for rank={self.rank} world={self.world}.")

    def _to_feature(self, sample):
        lat = sample.get(self.latent_key)
        if lat is None:
    ...
    def __iter__(self):
        ds = wds.WebDataset(
            self._my_shards, shardshuffle=int(self.shardshuffle),
            handler=wds.warn_and_continue, nodesplitter=None,
            workersplitter=wds.split_by_worker, empty_check=False,
        )
        ...
        ds = ds.map(self._to_feature, handler=wds.warn_and_continue).repeat()
...
def train():
    ...
    dataset = WDSLatentsIterableDataset(
        ...
        latent_key=data_args.latent_key, caption_key=data_args.caption_key,
        meta_key=data_args.meta_key, meta_input_ids_field=data_args.meta_input_ids_field,
        use_precomputed_ids=data_args.use_precomputed_ids,
        max_caption_tokens=data_args.max_caption_tokens,
        shardshuffle=data_args.shardshuffle, shuffle_buffer=data_args.shuffle_buffer,
        max_samples=data_args.max_samples, seed=training_args.seed,
    )
    ...
    if model_args.init_model_dir:
        ...
        cfg = OneFlowConfig(
            ...
            dim=model_args.dim, depth=model_args.depth,
            dim_head=model_args.dim_head, heads=model_args.heads,
            dim_latent=model_args.dim_latent,
        )
        model = OneFlowModel(cfg)

    accelerate.PartialState().wait_for_everyone()
    logger.info("Start OneFlow mixed-generation training...")
    trainer = OneFlowMixedGenerationTrainer(
