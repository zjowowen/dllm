"""
OneFlow interleaved-generation pretraining from offline WebDataset latents shards.

Full Algorithm 3: τ_text ~ Unif[0,2] with interleaved image schedule.

Corresponds to Phase 2b in doc/oneflow/text_image_interleaved_review_zh.md.

Example:
  accelerate launch --config_file scripts/accelerate_configs/npu_ddp.yaml \
    examples/oneflow_interleaved/pt_interleaved.py \
    --output_dir /tmp/oneflow_interleaved \
...
import dllm
from dllm.pipelines.oneflow_interleaved.trainer import OneFlowInterleavedTrainer
...
@dataclass
class TrainingArguments(OneFlowInterleavedTrainer.OneFlowInterleavedConfig):
    output_dir: str = None
...
def train():
    ...
    logger.info("Start OneFlow interleaved-generation training (full Algorithm 3)...")
    trainer = OneFlowInterleavedTrainer(
