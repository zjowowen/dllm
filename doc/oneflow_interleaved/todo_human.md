...
实验进展记录在 `doc/oneflow_interleaved/PROGRESS.md` 中。
...
**执行命令**：
```bash
bash scripts/oneflow_interleaved/launch_pt_interleaved_910c.sh \
  --shards "data/latents_128_bundle/wds_latents_flower32" \
  --output_dir data/ckpts/interleaved_2b1_baseline \
  --max_steps 5000 \
  --tau_text_max 2.0 \
...
bash scripts/oneflow_interleaved/launch_pt_interleaved_910c.sh \
  --shards "data/latents_128_bundle/wds_latents_flower32" \
  --output_dir data/ckpts/interleaved_2b3_paper \
  --max_steps 5000 \
  --tau_text_max 2.0 \
  --image_loss_weight 1.0 \
...
python -u examples/oneflow_interleaved/sample_interleaved.py \
  --model_dir data/ckpts/interleaved_2b1_baseline/checkpoint-best \
  --mode text_only \
  --prompt "OneFlow is a generative model" \
  --max_new_tokens 128 \
  --output_dir outputs/interleaved_3a1_text
```
...
python -u examples/oneflow_interleaved/sample_interleaved.py \
  ...
  --output_dir outputs/interleaved_3b1_image
```
...
python -u examples/oneflow_interleaved/sample_interleaved.py \
  ...
  --output_dir outputs/interleaved_3c1_full
```
...
pytest -q scripts/tests/test_oneflow_sequence_ops.py -k "train_vs_sampler_consistency"
```
