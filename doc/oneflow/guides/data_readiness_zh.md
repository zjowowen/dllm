...
|---|---|---|---|---|---|---|
| **1a text-only** | 文本 PT | HF Arrow (`input_ids`) | `data/offline/pt_text_dclm_1024_12k` | 12K seq, len=1024 | N/A | OK |
| 1a text-only (大) | 文本 PT | HF Arrow | `data/offline/pt_text_fineweb_1024_100k` | 100K seq, len=1024 | N/A | OK |
| 1a text-only (overfit) | 文本 PT | HF Arrow | `data/offline/pt_text_dclm_1024_overfit256` | 256 seq | N/A | OK |
| **1b image-only** | WDS latents | tar (npy+txt+json) | `data/latents_128_bundle/wds_latents_flower32` | ~subset, 128×128 | 16 | OK (单节点) |
| 1b image-only (全量) | WDS latents | tar (npy+txt+json) | `data/latents_128_bundle/wds_latents_4096` | 67K, 128×128 | 17 | OK (单节点) |
...
| 阶段 | 资源类型 | 路径 | 状态 |
|---|---|---|---|
| 1a text-only | 评估提示词 (minimal) | `scripts/oneflow/eval_prompts_text_minimal.jsonl` | OK (6 条) |
| 1a text-only | 评估提示词 (tiered) | `scripts/oneflow/eval_prompts_text_tiered_v1.jsonl` | OK (16 条) |
| 1a text-only | 批量评估脚本 | `scripts/oneflow/eval_text_only_prompts.py` | OK |
| 1a text-only | 离线 loss 评估 | `scripts/oneflow/eval_text_only_loss.py` | OK |
| 1a text-only | checkpoint sweep | `scripts/oneflow/eval_text_only_checkpoint_sweep.py` | OK |
| 1b image-only | 图像评估提示词 | `scripts/oneflow/eval_prompts_image_v1.jsonl` | **需创建** |
| 1b image-only | 图像采样评估脚本 | `scripts/oneflow_image_only/eval_image_sample.py` | **需创建** |
| 1b image-only | 过拟合评估 | `examples/oneflow/overfit_eval_wds.py` | OK |
| 2a mixed | 混合评估提示词 | `scripts/oneflow/eval_prompts_mixed_v1.jsonl` | **需创建** |
| 2b interleaved | 三模式采样脚本 | `examples/oneflow_interleaved/sample_interleaved.py` | **需创建** |
| 通用 | latent 解码验证 | `scripts/oneflow/verify_wds_latents_decode.py` | OK |
| 通用 | 单条采样 + VAE decode | `examples/oneflow/sample_and_decode.py` | OK |

### 1.3 Tokenizer 状态

| 路径 | 基底模型 | 特殊 Token | 状态 |
|---|---|---|---|
| `data/latents_128_bundle/tokenizer` | GPT-2 | `<\|oneflow_image\|>`, `<\|oneflow_image_som\|>`, `<\|oneflow_image_eom\|>` | OK |
...
**当前数据**:
- `data/latents_128_bundle/wds_latents_flower32` (16 shards, 花朵子集)
- `data/latents_128_bundle/wds_latents_4096` (17 shards, 67K CC3M 样本)
...
- 训练脚本通过 `--mixed_generation_prob` 控制每个 batch 中图像样本的比例
- 100% 的样本都来自 WDS shards（含图文对），由训练脚本在运行时随机决定是否 mask 掉图像
...
一键脚本: `scripts/oneflow/reshard_for_multinode.sh`

```bash
bash scripts/oneflow/reshard_for_multinode.sh
```
...
python -u scripts/oneflow/reshard_wds.py \
  ...
  --output_dir data/latents_128_bundle/wds_latents_mn \
  --maxcount 512
```
...
ls data/latents_128_bundle/wds_latents_mn/*.tar | wc -l
```

### 3.5 使用 resharded 数据训练

在 N-node 脚本中将 `--shards` 改为 resharded 目录:
```bash
bash scripts/oneflow_image_only/launch_pt_image_910c_nnode.sh \
...
| 资源 | 路径 | 说明 |
|---|---|---|
| 提示词 (minimal) | `scripts/oneflow/eval_prompts_text_minimal.jsonl` | 6 条，含 expected_contains |
| 提示词 (tiered) | `scripts/oneflow/eval_prompts_text_tiered_v1.jsonl` | 16 条，分层难度 |
| 批量评估 | `scripts/oneflow/eval_text_only_prompts.py` | JSONL → report.md |
| Loss 评估 | `scripts/oneflow/eval_text_only_loss.py` | 多 seed 离线 loss |
| Sweep | `scripts/oneflow/eval_text_only_checkpoint_sweep.py` | 多 checkpoint 排名 |
...
| 资源 | 路径 | 说明 |
|---|---|---|
| 提示词 | `scripts/oneflow/eval_prompts_image_v1.jsonl` | 16 条，涵盖花朵 / 自然 / 物体 / 抽象 |
| 批量采样 | `scripts/oneflow_image_only/eval_image_sample.py` | 批量采样 + VAE decode + report |
| 过拟合评估 | `examples/oneflow/overfit_eval_wds.py` | GT vs Gen 对比 + MSE/PSNR |
| 单条采样 | `examples/oneflow/sample_and_decode.py` | 单条提示 → 图像 |
| latent 验证 | `scripts/oneflow/verify_wds_latents_decode.py` | 验证 latent shard decode 质量 |
...
| 资源 | 路径 | 说明 |
|---|---|---|
| 提示词 | `scripts/oneflow/eval_prompts_mixed_v1.jsonl` | 16 条，含纯文本和图像生成提示 |
...
| 资源 | 路径 | 说明 |
|---|---|---|
| 三模式采样 | `examples/oneflow_interleaved/sample_interleaved.py` | text_only / image_conditioned / interleaved |
...
### Phase 1a (Text-only)
- [ ] `data/offline/pt_text_*` 目录存在且非空
- [ ] `dataset/train/dataset.arrow` 文件可读
- [ ] tokenizer 与数据 tokenize 时使用的一致
- [ ] `eval_prompts_text_*.jsonl` 格式正确

### Phase 1b (Image-only)
- [ ] `data/latents_128_bundle/wds_latents_flower32/*.tar` 存在
- [ ] `data/latents_128_bundle/tokenizer/` 可读
...
| 脚本 | 功能 |
|---|---|
| `scripts/oneflow/hf_export_urls_tsv.py` | HF 元数据 → urls.tsv |
| `scripts/oneflow/precompute_latents_wds.py` | 图像 → VAE latent WDS shards |
| `scripts/oneflow/reshard_wds.py` | 重分片（不重算 latent） |
| `scripts/oneflow/reshard_for_multinode.sh` | 一键多节点 resharding |
| `scripts/oneflow/prepare_pt_text_dataset.py` | 文本 tokenize → HF Arrow |
| `scripts/oneflow/prepare_pt_bundle.sh` | 文本 PT 数据准备 wrapper |

### 数据验证
| 脚本 | 功能 |
|---|---|
| `scripts/oneflow/verify_wds_latents_decode.py` | 解码 latent 验证质量 |

### 评估
| 脚本 | 功能 |
|---|---|
| `scripts/oneflow/eval_text_only_prompts.py` | 文本提示批量评估 |
| `scripts/oneflow/eval_text_only_loss.py` | 离线 loss 评估 |
| `scripts/oneflow/eval_text_only_checkpoint_sweep.py` | 多 checkpoint 排名 |
| `scripts/oneflow_image_only/eval_image_sample.py` | 图像采样 + VAE decode 批量评估 |
| `examples/oneflow/overfit_eval_wds.py` | GT vs Gen 过拟合评估 |
| `examples/oneflow/sample_and_decode.py` | 单条采样 + VAE decode |
| `examples/oneflow_interleaved/sample_interleaved.py` | Interleaved 三模式采样 |
