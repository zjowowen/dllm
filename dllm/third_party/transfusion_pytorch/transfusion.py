...
"""
Vendored from `transfusion-pytorch` (MIT License).

Upstream repository:
  https://github.com/lucidrains/transfusion-pytorch

See `dllm/third_party/transfusion_pytorch/LICENSE` and `NOTICE.md` for attribution.
...
class Transformer(Module):
    ...
    def forward(
        ...
        if needs_masking:
            # Allow callers to pass a manual attention mask (e.g., for padding).
            # NOTE: upstream code path did not thread `attn_mask` through unless modality_positions were set.
            if exists(attn_mask):
                assert not should_use_flex_attn, (
                    "Passing a manual `attn_mask` is not supported when flex attention is enabled."
                )
